...
class Section implements JsonSerializable{
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name from {$tx}sections $criteria limit $top,$perpage");
		$data=[];
		while($section=$result->fetch_object()){
			$data[]=$section;
		}
	...
	public static function count($criteria=""){
		global $db,$tx;
		$result =$db->query("select count(*) from {$tx}sections $criteria");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		global $db,$tx;
		$count_result =$db->query("select count(*) total from {$tx}sections $criteria ");
		list($total_rows)=$count_result->fetch_row();
		$total_pages = ceil($total_rows /$perpage);
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,name from {$tx}sections $criteria limit $top,$perpage");
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-warning\" href=\"create-section\"><b>New Section</b></a></th></tr>";
		if($action){
		...
		while($section=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= action_button(["id"=>$section->id, "name"=>"Details", "value"=>"Details", "class"=>"info", "url"=>"details-section"]);
