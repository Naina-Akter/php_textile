...
";
	...
	static function html_table($page = 1,$perpage = 10,$action=true){
		...
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-warning\" href=\"create-order\">New Order</a></th></tr>";
		if($action){
		...
		while($order=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= action_button(["id"=>$order->id, "name"=>"Details", "value"=>"Invoice", "class"=>"info", "url"=>"details-order"]);
