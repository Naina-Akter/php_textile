...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		//select s.id,p.name product,s.qty,tt.name transactions,s.remark,s.created_at,w.name warehouse from core_stocks s,core_products p,core_transaction_types tt,core_warehouses w where s.product_id=p.id and s.transaction_type_id=tt.id and s.warehouse_id=w.id 
		$result=$db->query("select s.id,p.name product,s.qty,tt.name transactions,s.remark,s.created_at,w.name warehouse from core_stocks s,core_products p,core_transaction_types tt,core_warehouses w where s.product_id=p.id and s.transaction_type_id=tt.id and s.warehouse_id=w.id $criteria limit $top,$perpage");
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-warning\" href=\"create-stock\"><b>New Stock</b></a></th></tr>";
		if($action){
		...
		while($stock=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= action_button(["id"=>$stock->id, "name"=>"Edit", "value"=>"Edit", "class"=>"warning", "url"=>"edit-stock"]);
				$action_buttons.= action_button(["id"=>$stock->id, "name"=>"Delete", "value"=>"Delete", "class"=>"info", "url"=>"stocks"]);
				$action_buttons.= "</div></td>";
			}
			$html.="<tr><td>$stock->id</td><td>$stock->product</td><td>$stock->qty</td><td>$stock->transactions</td><td>$stock->remark</td><td>$stock->created_at</td><td>$stock->warehouse</td> $action_buttons</tr>";
		}
		$html.="</table>";
