...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select p.id,s.name supplier,p.purchase_date,p.delivery_date,p.shipping_address,p.purchase_total,p.paid_amount,p.remark,ss.name status,p.discount,p.vat from core_purchases p,core_suppliers s,core_status ss where p.supplier_id=s.id and p.status_id=ss.id $criteria limit $top,$perpage");
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-warning\" href=\"create-purchase\"><b>New Purchase</b></a></th></tr>";
		if($action){
		...
		while($purchase=$result->fetch_object()){
			...
			$html.="<tr><td>$purchase->id</td><td>$purchase->supplier</td><td>$purchase->purchase_date</td><td>$purchase->delivery_date</td><td>$purchase->shipping_address</td><td>$purchase->purchase_total</td><td>$purchase->paid_amount</td><td>$purchase->remark</td><td>$purchase->status</td><td>$purchase->discount</td><td>$purchase->vat</td> $action_buttons</tr>";
		}
		$html.="</table>";
