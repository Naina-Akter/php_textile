...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-warning\" href=\"create-bom\"><b>New BoM</b></a></th></tr>";
		if($action){
		...
		while($mfgbom=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= action_button(["id"=>$mfgbom->id, "name"=>"Details", "value"=>"Details", "class"=>"secondary", "url"=>"details-bom"]);
				$action_buttons.= action_button(["id"=>$mfgbom->id, "name"=>"Edit", "value"=>"Edit", "class"=>"primary", "url"=>"edit-bom"]);
				$action_buttons.= action_button(["id"=>$mfgbom->id, "name"=>"Delete", "value"=>"Delete", "class"=>"danger", "url"=>"MfgBom"]);
				$action_buttons.= action_button(["id"=>$mfgbom->id, "name"=>"Production", "value"=>"Production", "class"=>"primary", "url"=>"create-production"]);
